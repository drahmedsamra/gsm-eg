...
export function CourseCard({ course }: CourseCardProps) {
  ...
  return (
    <article
      className={cn(
        "group relative flex flex-col overflow-hidden rounded-2xl border border-gsm-navy/8 bg-white shadow-lg transition-all duration-300",
    ...
    >
      <div className="relative aspect-[16/10] overflow-hidden">
        ...
        />
        <div className="absolute inset-0 bg-gradient-to-t from-gsm-navy/60 via-transparent to-transparent" />
      ...
      <div className="flex flex-1 flex-col p-6">
        <h3 className="text-xl font-bold text-gsm-navy">{title}</h3>
        ...
        <dl className="mt-4 flex gap-4 text-xs text-gsm-muted">
          ...
          <div>
            <dt className="sr-only">{t("level")}</dt>
            <dd>{pick(locale, course.level)}</dd>
          </div>
        </dl>
        <Button
